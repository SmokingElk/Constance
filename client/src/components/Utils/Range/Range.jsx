...
class Range extends React.Component {
    ...
    calcThumbOffset (valueMapped) {
        let maxWidth = 0;
        if (this.container.current) maxWidth = this.container.current.getBoundingClientRect().width - 20;
        
        return valueMapped * maxWidth + "px";
    }
    
    render () {
        let valueMapped = Math.min(1, Math.max(0, (this.props.value - this.props.min) / (this.props.max - this.props.min)));
        ...
        return (
            ...
            ref={this.container}>
                ...
                style={{left: this.calcThumbOffset(valueMapped)}}></div>
            </div>
        );
    }
}
